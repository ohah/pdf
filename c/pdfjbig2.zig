...
//
// 여기서 다루는 것: 보통 영역(산술·MMR), 글자 사전(산술·허프만), 글자
// 영역, 세밀화(refinement), 무늬 사전과 하프톤, 문서가 실어 온 허프만 표.
// 곧 T.88 부록 H 시험 흐름이 요구하는 것은 다 있다.
//
// 다루지 않는 것: 줄무늬로 끊어 보내는 쪽(striped page), 미리 알 수 없는
// 길이(0xFFFFFFFF)로 온 영역.
...
fn gatherRefs(refs: []const u32) void {
    ...
    // 가리키는 것이 *있는데* 못 찾았을 때만 지금까지 모인 것을 다 쓴다.
    //
    // 아예 안 가리키는 사전은 물려받는 글자가 없는 것이 맞다. 그걸 "다 쓴다"
    // 로 봤더니, 앞 사전의 글자를 물려받아 제 것인 양 다시 내보냈다. 목록에
    // 같은 글자가 두 번 들어가니 글자 번호가 하나씩 밀린다 — 부록 H 쪽1·쪽2
    // 에서 다섯 글자 중 둘이 딴 모양으로 나왔다(허프만·산술 두 길 모두).
    if (inlist_n == 0 and refs.len > 0) {
        var k: u32 = 0;
