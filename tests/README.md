...
## tests/mkgap.mjs · tests/gap.mjs — 아직 없는 기능을 못 박기

    node tests/mkgap.mjs tests/fixtures
    node tests/gap.mjs tests/fixtures
    node tests/gap.mjs tests/fixtures --update   # 기능을 넣어 동작이 바뀌었을 때

앞의 시험들과 성격이 다르다. 저기는 "poppler·pdf.js 와 화소가 같아야 한다"
를 단언한다. 여기는 그럴 수 없다 — 아직 안 만든 기능이라 첫날부터 빨간불이
되고, 그러면 신호가 무뎌진다.

그래서 셋만 본다: **안 죽는다 · 두 번 그려 같다 · 지금 무엇을 하는가**.
마지막은 일부러 변경 감지기다.

왜 미리 만드나. 산문으로 적은 "우리는 X 를 안 한다" 는 썩는다 —
`pdfjbig2.zig` 머리 주석이 "세밀화·하프톤·허프만 사전은 안 다룬다" 고 적어
둔 채로 셋 다 구현돼 있었다. 실행되는 견본은 안 썩고, 누가 그 기능을 넣는
날 계약서가 이미 있다.

지금 담은 빈틈: `/BS`(테두리) · `/TI`(리스트 첫 줄) · `/W2`·`/DW2`(세로
이동량) · `/Matte`(가리개 바탕색) · `/Interpolate`(확대 시 부드럽게) ·
